  ...
  //加载页面邦定事件
    $(function () {
        ...
        $(window).resize(function () {
            ...
            // 获取登录信息的top、left定位
            loginInfo.style.top = loginTop.offsetHeight + imgTop.clientHeight + loginImg.offsetHeight * 0.87 + 'px';
            //loginInfo.style.left = pageWidth * 0.5 - loginImg.offsetWidth * 0.38 + 'px';
        })
...
<body>
    <div id="loginTop">
        <!-- 加载天气 -->
        <div id="topLeft">
            <iframe id="leftWeather" scrolling="no" allowtransparency="true" src="http://i.tianqi.com/index.php?c=code&id=1&num=1"></iframe>
        </div>
        <!-- 批量上传下载工具、后台登录、联系我们 -->
        <div id="topRight">
            <span id="rightAbout"> 
				<!--<a href="<%=path%>/download/BulkImporter&Exporter.rar"	target='_blank'><fmt:message key="cosog.upload"/></a>&nbsp;|&nbsp; -->
				<a href="<%=path%>/login/toBackLogin"><fmt:message key="cosog.backLogin"/></a>&nbsp;|&nbsp;
				<a href="http://www.cosogoil.com/" target='_blank'><fmt:message key="cosog.contact"/></a> 
            </span>
        </div>
    </div>
    <div id="loginBody">
        <div id="imgTop"></div>
        <img id="loginImg" src="<%=path%>/images/login/01.png"/>
        <img id="" src="<%=path%>/images/login/02.png" style="display:none;"/>
        <!-- 推荐使用谷歌浏览器 -->
        <div id="imgBottom">
        ...
        <div id="imgTitle">
            <img id="logoImg" src="<%=path%>/images/logo/ytlogo.png" style="display:block;" />&nbsp;
            <span id="titleText"><fmt:message key="cosog.title"/></span>
        </div>
        <!-- 登录信息 -->
        <div id="loginInfo">
            <span><fmt:message key="cosog.username"/>：</span>
            <input type="text" id="userId" class="input" value="<%=name%>"/>&nbsp;
            <span><fmt:message key="cosog.password"/>：</span>
            <input type="password" id="userPwd" class="input" value="<%=password%>" onkeypress="if(event.keyCode==13){loginFn();}"/>&nbsp;
            <label>
            ...
            <span><fmt:message key="cosog.rememberpassword"/></span>&nbsp;
            <button type="button" class="button" onclick="loginFn();"><span aria-hidden="true" data-icon="&#xf007;">&nbsp;&nbsp;<fmt:message key="cosog.login"/></span></button>&nbsp;
            <button type="button" onclick="loginAlarmFn();" style="display:none;"><span>&nbsp;<fmt:message key="cosog.loginalarm"/></span></button>
