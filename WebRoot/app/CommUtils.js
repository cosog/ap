...
graghMinWidth = 300;
defaultJhComboxSize=10000;
comboxPagingStatus=0;//0-不分页  大于0分页
defaultJhhComboxSize=10000;
 ...
 * 获取图形即时查询记录数
 * **/ 
var showInstantTotalsCount=function(orgId,page,type,jh,url,countId){
	// 检索动态列表头及内容信息
		Ext.Ajax.request({
			method : 'POST',
			url : context+ url,
			...
			success : function(response, opts) {
				...
				var obj = Ext.decode(response.responseText);
				if(isNotVal(obj)){
				Ext.getCmp(countId).setValue(obj.totalCount);
				}
				// ==end
			},
			failure : function(response, opts) {
 ...
 * 获取图形历史查询记录数
 * **/ 
var showDesignTimeTotalsCount=function(orgId,page,type,jh,fromDate,endDate,url,countId){
	// 检索动态列表头及内容信息
		Ext.Ajax.request({
			method : 'POST',
			url : context+ url,
 ...
 * 获取图形全天查询记录数
 * **/ 
var showAlldayTotalsCount=function(orgId,page,type,jh,gtcjsj,url,countId){
	// 检索动态列表头及内容信息
		Ext.Ajax.request({
			method : 'POST',
			url : context+ url,
 ...
 * Curver chart
 */
ScadaCurverVFnChartFn = function(store, w, h,jh,cjsj,stroke,spm,divid,title,color,yname) {
	var items = store.data.items;
	var data = "[";
	if(items.length>0){
		for (var i = 0; i <= items.length; i++) {
			if(i<items.length){
				data += "[" + items[i].data.XData + ","+items[i].data.YData+"],";
			}else{
				data += "[" + items[0].data.XData + ","+items[0].data.YData+"]";//将第一个点拼到最后面，使图形闭合
			}
		}
	}
	data+="]";
	var pointdata = Ext.JSON.decode(data);
	initScadaCurveChart(pointdata, w, h, jh, cjsj,stroke,spm,divid,title,color,yname);
	return false;
}


function initScadaCurveChart(data, w, h, jh, cjsj,stroke,spm,divid,title,color,yname) {
	mychart = new Highcharts.Chart({
				chart: {                                                                             
		            type: 'scatter',     // 散点图   
		            renderTo : divid,
...
showBalanceAnalysisCurveChart = function(store, divid1,divid2) {
	var currenttorquecurveArr=store.proxy.reader.rawData.currenttorquecurve;
	var optimizitiontorquecurveArr=store.proxy.reader.rawData.optimizitiontorquecurve;
	var legendName = ['载荷扭矩','平衡块扭矩','曲柄扭矩','净扭矩'];
	var catagories1 = "[";
    var series1 = "[";
    var catagories2 = "[";
    var series2 = "[";
    if(currenttorquecurveArr.length>0){
    	var loadData="{\"name\":\"载荷扭矩\",\"data\":[";
    	var balanceData="{\"name\":\"平衡块扭矩\",\"data\":[";
    	var crankData="{\"name\":\"曲柄扭矩\",\"data\":[";
    	var netData="{\"name\":\"净扭矩\",\"data\":[";
    	var currenttorquecurve=currenttorquecurveArr[0];
        var currenttorquecurvedata=currenttorquecurve.curvedata.split(";"); // 功图数据：功图点数， 冲次，冲程 位移1，载荷1，位移2，载荷2...
    ...
    if(optimizitiontorquecurveArr.length>0){
    	var loadData="{\"name\":\"载荷扭矩\",\"data\":[";
    	var balanceData="{\"name\":\"平衡块扭矩\",\"data\":[";
    	var crankData="{\"name\":\"曲柄扭矩\",\"data\":[";
    	var netData="{\"name\":\"净扭矩\",\"data\":[";
    	var optimizitiontorquecurve=optimizitiontorquecurveArr[0];
        var optimizitiontorquecurvedata=optimizitiontorquecurve.curvedata.split(";"); // 功图数据：功图点数， 冲次，冲程 位移1，载荷1，位移2，载荷2...
    	...
    	crankData+="]}";
    	netData+="]}";
    	series2+=loadData+","+balanceData+","+crankData+","+netData;
    }
	...
	var cat2 = Ext.JSON.decode(catagories2);
	var ser2 = Ext.JSON.decode(series2);
	initBalanceCurveChart(cat1,ser1, divid1,"目前扭矩曲线","扭矩(kN*m)","曲柄转角(度)");
	initBalanceCurveChart(cat2,ser2, divid2,"预期扭矩曲线","扭矩(kN*m)","曲柄转角(度)");
	return false;
}
//抽油机运动特性曲线和光杆位置扭矩因数表
showBalanceAnalysisMotionCurveChart = function(store, panelid,gridid,divid) {
	var get_rawData = store.proxy.reader.rawData;
	var motioncurveArr=get_rawData.motioncurve;
	var arrColumns = get_rawData.columns2;
	var column = createBalanceMinitorListColumn(arrColumns)
    var newColumns = Ext.JSON.decode(column);
	var list=get_rawData.list;
	
	var MonitorTorqueMaxValueMotionCurveGrid = Ext.getCmp(gridid);
    if (isNotVal(MonitorTorqueMaxValueMotionCurveGrid)) {
    	Ext.getCmp(panelid).remove(MonitorTorqueMaxValueMotionCurveGrid);
    }
	
	Ext.create('Ext.data.Store', {
	    storeId: 'torqueMaxValuePRAndTFStore',
	    fields:[ 'crankAngle', 'PR', 'TF'],
	    data:list
	});
	
	MonitorTorqueMaxValueMotionCurveGrid = Ext.create('Ext.grid.Panel', {
        id: gridid,
        border: false,
        autoLoad: false,
        columnLines: true,
        ...
        store: Ext.data.StoreManager.lookup('torqueMaxValuePRAndTFStore'),
        columns: newColumns
    });
    Ext.getCmp(panelid).add(MonitorTorqueMaxValueMotionCurveGrid);
	
	var catagories = "[";
    var series = "[";
    if(motioncurveArr.length>0){
    	var sData="{\"name\":\"位移\",\"yAxis\":0,\"data\":[";
    	var vData="{\"name\":\"速度\",\"yAxis\":1,\"data\":[";
    	var aData="{\"name\":\"加速度\",\"yAxis\":2,\"data\":[";
    	var motioncurve=motioncurveArr[0];
        var motioncurvedata=motioncurve.curvedata.split(";"); // 功图数据：功图点数， 冲次，冲程 位移1，载荷1，位移2，载荷2...
        for(var i=0;i<motioncurvedata.length;i++){
        	var everyData=motioncurvedata[i].split(",");
        	catagories+=everyData[0];
        	sData+=everyData[1];
        	vData+=everyData[2];
        	aData+=everyData[3];
        	if(i<motioncurvedata.length-1){
        		catagories+=",";
        		sData+=",";
        		vData+=",";
        		aData+=",";
    ...
    var cat = Ext.JSON.decode(catagories);
	var ser = Ext.JSON.decode(series);
	initBalanceCurveChartThreeY(cat,ser, divid,"抽油机运动特性曲线","值","曲柄转角(度)");
	return false;
}
 ...
 * 拼接地面功图曲线数据
 */
showBalanceAnalysisSurfaceCardChart = function(store, divid,jh) {
    var cjsj='';
    var strokestroke;
    var spm='';
    var phd=store.proxy.reader.rawData.currentdegreeofbalance;
    var data = "["; // 功图data
    var gtdataArr=store.proxy.reader.rawData.gtsj;
    if(gtdataArr.length>0){
    	var gtdata=gtdataArr[0];
        var gt=gtdata.gtsj.split(","); // 功图数据：功图点数， 冲次，冲程 位移1，载荷1，位移2，载荷2...
        var gtcount=gt[2]; // 功图点数
        cjsj=gtdata.cjsj;
        stroke=gtdata.stroke;
        spm=gtdata.spm;
    	if(gt.length>0){
	...
	var pointdata = Ext.JSON.decode(data);
	//画功图
	initBalanceSurfaceCardChart(jh,pointdata, cjsj,stroke,spm,phd, divid);
	return false;
}
 ...
 * 拼接地面功图曲线数据
 */
showBalanceCycleSurfaceCardChart = function(gtdata, divid,jh) {
    var cjsj='';
    var stroke='';
    var spm='';
    var phd='';
 ...
 * li 2015-07-23
 */
showContrastSurfaceCardChart = function(jh,csrq,djgt,divid) {
	var color=new Array("#000000","#00ff00"); // 线条颜色
    var djgtsj = djgt.split("#");
	var series = "[{";
	for (var i =0; i < djgtsj.length; i++){
		var gtsj = djgtsj[i].split(","); // 功图数据：功图点数，位移1，载荷1，位移2，载荷2...
        var gtcount = gtsj[0];
		var data = "[";
		...
		data+="]";
		if(djgtsj.length==1){
			series+="name: ''," + "color: '" + color[0] + " ' , " + "lineWidth:3," + "data:" + data + "}";
		}else{
			if(i==0){
				series+="name: ''," + "color: '" + color[0] + " ' , " + "lineWidth:3," + "data:" + data + "},{";
			}else if((i>0)&&(i<(djgtsj.length-1))){
	            series+="name: ''," + "color: '" + color[1] + " ' , " + "lineWidth:2," + "data:" + data + "},{";
	        }else{
				series+="name: ''," + "color: '" + color[1] + " ' , " + "lineWidth:2," + "data:" + data + "}";
			}
		}
...
//	}
	...
	title = '';
	initMultiSurfaceCardChart(pointdata, title,jh, csrq, divid);
	return false;
}
 ...
 * li 2015-07-23
 */
function initMultiSurfaceCardChart(series, title, jh, cjsj, divid,upperLoadLine,lowerLoadLine) {
	mychart = new Highcharts.Chart({
				chart: {                                                                             
		            type: 'scatter',      // 散点图   
		            renderTo : divid,
...
//	Highcharts.setOptions({                     // 设置默认颜色
//		colors: ['#00FF00','#006837', '#00FF00','#006837', '#00FF00','#006837']
//	});
	mychart = new Highcharts.Chart({
				chart: {                                                                             
		            type: 'column',                      // 柱状图
 ...
 * 拼接单井信息曲线
 */
showHistoryWellMixCurve = function(items, divid,jh) {
	var axdyssz = [],
		bxdyssz = [],
		cxdyssz=[],
		sxpjdyssz=[],
	...
	for(var i=0;i<items.length;i++){
		var j=items.length-1-i;
		axdyssz.push([
		               Date.parse(items[j].cjsj.replace(/-/g, '/')),
		               parseFloat(items[j].axdyssz)
		]);
		bxdyssz.push([
		               Date.parse(items[j].cjsj.replace(/-/g, '/')),
		               parseFloat(items[j].bxdyssz)
		]);
		cxdyssz.push([
		               Date.parse(items[j].cjsj.replace(/-/g, '/')),
		               parseFloat(items[j].cxdyssz)
		]);
//		sxpjdyssz.push([
//		               Date.parse(items[j].cjsj.replace(/-/g, '/')),
//		               parseFloat(items[j].sxpjdyssz)
//		]);
		axdlssz.push([
		              Date.parse(items[j].cjsj.replace(/-/g, '/')),
		              parseFloat(items[j].axdlssz)
	    ]);
		bxdlssz.push([
	               Date.parse(items[j].cjsj.replace(/-/g, '/')),
	               parseFloat(items[j].bxdlssz)
	    ]);
		cxdlssz.push([
	               Date.parse(items[j].cjsj.replace(/-/g, '/')),
	               parseFloat(items[j].cxdlssz)
	    ]);
//		sxpjdlssz.push([
//	               Date.parse(items[j].cjsj.replace(/-/g, '/')),
//	               parseFloat(items[j].sxpjdlssz)
//	    ]);
//		axygglssz.push([
//	               Date.parse(items[j].cjsj.replace(/-/g, '/')),
//	               parseFloat(items[j].axygglssz)
//	    ]);
//		bxygglssz.push([
//	               Date.parse(items[j].cjsj.replace(/-/g, '/')),
//	               parseFloat(items[j].bxygglssz)
//	    ]);
//		cxygglssz.push([
//	               Date.parse(items[j].cjsj.replace(/-/g, '/')),
//	               parseFloat(items[j].cxygglssz)
//	    ]);
//		sxzygglssz.push([
//			       Date.parse(items[j].cjsj.replace(/-/g, '/')),
//			       parseFloat(items[j].sxzygglssz)
//			    ]);
	}
	initHistoryWellMixCurve(axdyssz, bxdyssz,cxdyssz,sxpjdyssz,axdlssz, bxdlssz,cxdlssz,sxpjdlssz,axygglssz, bxygglssz,cxygglssz,sxzygglssz, divid,jh);
	return false;
}

function initHistoryWellMixCurve(axdyssz, bxdyssz,cxdyssz,sxpjdyssz,axdlssz, bxdlssz,cxdlssz,sxpjdlssz,axygglssz, bxygglssz,cxygglssz,sxzygglssz, divid,jh) {
	Highcharts.setOptions({
        global: {
            useUTC: false
        }
	...
	mychart = new Highcharts.StockChart({
		chart: {
			//backgroundColor:'#9bb2d2',
			//plotBackgroundColor:'#bac5d3',
            renderTo : divid
        }, 
        colors: ['#e81b50', '#3da904', '#4d4749'],
//        exporting: {    
...
//历史查询电参曲线画图方法
showHistoryElecGraphical = function(graphicalList, divId,title,ytitle) {
	var catagories = "[";
	var series = "[";
	if(graphicalList!=undefined){
		var str =graphicalList.cjsj;
		str = str.replace(/-/g,"/");
		var date = new Date(str );
		var AdataArr=graphicalList.Adata.split(",");
		var BdataArr=graphicalList.Bdata.split(",");
		var CdataArr=graphicalList.Cdata.split(",");
//		var AvgdataArr=graphicalList.Avgdata.split(",");
		
		for (var i = 0; i < AdataArr.length-2; i++) {
			var newdate=new Date(date.getTime()+i*parseInt(AdataArr[1]));
			var month=newdate.getMonth() + 1;
			catagories += "\"" + newdate.getFullYear()+"-"+month+"-"+newdate.getDate()+" "+newdate.getHours()+":"+newdate.getMinutes()+":"+newdate.getSeconds() + "\"";
			if (i < AdataArr.length - 3) {
				catagories += ",";
			}
		}
		...
		for (var i = 0; i < legendName.length; i++) {
			...
			series += "\"data\":[";
			for (var j = 2; j < AdataArr.length; j++) {
				if (i == 0) {
					series += AdataArr[j];
				} else if (i == 1) {
					series += BdataArr[j];
...
//				}
	...
	var cat = Ext.JSON.decode(catagories);
	var ser = Ext.JSON.decode(series);
	initHistoryElecGraphicalY(cat, ser, divId,graphicalList,title,ytitle);
	return false;
}

function initHistoryElecGraphical(years, values, divId,graphicalList,title,ytitle) {
	var jh=graphicalList.jh;         // 井名
	var cjsj=graphicalList.cjsj;     // 采集时间
	
	var maxA=graphicalList.maxA;     // A相最大
	var minA=graphicalList.minA;     // A相最小
	var avgA=graphicalList.avgA;     // A相平均
	
	var maxB=graphicalList.maxB;     // B相最大
	var minB=graphicalList.minB;     // B相最小
	var avgB=graphicalList.avgB;     // B相平均
	
	var maxC=graphicalList.maxC;     // C相最大
	var minC=graphicalList.minC;     // C相最小
	var avgC=graphicalList.avgC;     // C相平均
	var xtext='<p style="text-align:center;">采集时间</p>';
    xtext+='<p>A相最大:' + maxA + ',B相最大:' + maxB + ',C相最大:' + maxC + '</p>';
    xtext+='<p>A相最小:' + minA + ',B相最小:' + minB + ',C相最小:' + minC + '</p>';
    xtext+='<p>A相平均:' + avgA + ',B相平均:' + avgB + ',C相平均:' + avgC + '</p>';
	mychart = new Highcharts.Chart({
				chart : {
					renderTo : divId
				},
...
function initHistoryElecGraphicalY(years, values, divId,graphicalList,title,ytitle) {
	var ytext='';
	var jh='';         // 井名
	var cjsj='';     // 采集时间
	...
	var avgC='';     // C相平均
	if(graphicalList!=undefined){
		jh=graphicalList.jh;         // 井名
		cjsj=graphicalList.cjsj;     // 采集时间
		
		maxA=graphicalList.maxA;     // A相最大
		minA=graphicalList.minA;     // A相最小
		avgA=graphicalList.avgA;     // A相平均
		
		maxB=graphicalList.maxB;     // B相最大
		minB=graphicalList.minB;     // B相最小
		avgB=graphicalList.avgB;     // B相平均
		
		maxC=graphicalList.maxC;     // C相最大
		minC=graphicalList.minC;     // C相最小
		avgC=graphicalList.avgC;     // C相平均
		ytext+='<br>A相最大:' + maxA + '<br>';
	    ytext+='A相最小:' + minA + '<br>';
	    ytext+='A相平均:' + avgA + '<br>';
	...
	mychart = new Highcharts.Chart({
		        ...
		        yAxis: [{
		        	...
		        	title:{
		        		...
		        		style:{
		        			color:'#a7a5a5',
		        			//font:'italic  12px/40px arial,sans-serif'
		        			fontFamily:'Arial',         //（字体族）: "Arial"、"Times New Roman"、"宋体"、"黑体"等
		        			fontStyle:'normal',       //（字体样式）: normal（正常）、italic（斜体）或oblique（倾斜）
		        			fontVariant:'normal',     //(字体变化): normal（正常）或small-caps（小体大写字母）;
		        			fontWeight:'normal',      //(字体浓淡): 是normal（正常）或bold（加粗）。有些浏览器甚至支持采用100到900之间的数字（以百为单位）
		        			fontSize:'5px',          //（字体大小）: 可通过多种不同单位（比如像素或百分比等）来设置, 如：12xp，12pt，120%，1em
		        			lineHeight:'18px'         //行高
		        		}
		        	},
		        	tickPosition:'inside',
 ...
 * 拼接离散数据实时曲线数据信息
 */
showRealtimeDisperseCurve = function(store, divid) {
	var axdyssz = [],
		bxdyssz = [],
		cxdyssz=[],
		sxpjdyssz=[],
	...
	var axdysszArr = get_rawData.axdyssz.split(",");
	var bxdysszArr = get_rawData.bxdyssz.split(",");
	var cxdysszArr=get_rawData.cxdyssz.split(",");
//	var sxpjdysszArr=get_rawData.sxpjdyssz.split(",");
	var axdlsszArr=get_rawData.axdlssz.split(",");
	var bxdlsszArr=get_rawData.bxdlssz.split(",");
	var cxdlsszArr=get_rawData.cxdlssz.split(",");
//	var sxpjdlsszArr=get_rawData.sxpjdlssz.split(",");
	//var axygglsszArr=get_rawData.axygglssz.split(",");
	//var bxygglsszArr=get_rawData.bxygglssz.split(",");
	//var cxygglsszArr=get_rawData.cxygglssz.split(",");
	//var sxzygglsszArr=get_rawData.sxzygglssz.split(",");
	
	for(var i=0;i<cjsjArr.length;i++){
		var j=cjsjArr.length-1-i;
		if(cjsjArr[j]!=""&&cjsjArr[j]!=null){
			axdyssz.push([
			               Date.parse(cjsjArr[j].replace(/-/g, '/')),
			               parseFloat(axdysszArr[j])
			]);
			bxdyssz.push([
			               Date.parse(cjsjArr[j].replace(/-/g, '/')),
			               parseFloat(bxdysszArr[j])
			]);
			cxdyssz.push([
			               Date.parse(cjsjArr[j].replace(/-/g, '/')),
			               parseFloat(cxdysszArr[j])
			]);
//			sxpjdyssz.push([
//			               Date.parse(cjsjArr[j].replace(/-/g, '/')),
//			               parseFloat(sxpjdysszArr[j])
//			]);
			axdlssz.push([
			              Date.parse(cjsjArr[j].replace(/-/g, '/')),
			              parseFloat(axdlsszArr[j])
		    ]);
			bxdlssz.push([
		               Date.parse(cjsjArr[j].replace(/-/g, '/')),
		               parseFloat(bxdlsszArr[j])
		    ]);
			cxdlssz.push([
		               Date.parse(cjsjArr[j].replace(/-/g, '/')),
		               parseFloat(cxdlsszArr[j])
		    ]);
//			sxpjdlssz.push([
//		               Date.parse(cjsjArr[j].replace(/-/g, '/')),
//		               parseFloat(sxpjdlsszArr[j])
//		    ]);
//			axygglssz.push([
//		               Date.parse(cjsjArr[j].replace(/-/g, '/')),
//		               parseFloat(axygglsszArr[j])
//		    ]);
//			bxygglssz.push([
//		               Date.parse(cjsjArr[j].replace(/-/g, '/')),
//		               parseFloat(bxygglsszArr[j])
//		    ]);
//			cxygglssz.push([
//		               Date.parse(cjsjArr[j].replace(/-/g, '/')),
//		               parseFloat(cxygglsszArr[j])
//		    ]);
//			sxzygglssz.push([
//				       Date.parse(cjsjArr[j].replace(/-/g, '/')),
//				       parseFloat(sxzygglsszArr[j])
//				    ]);
		}
	}
	initRealtimeDisperseCurve(axdyssz, bxdyssz,cxdyssz,sxpjdyssz,axdlssz, bxdlssz,cxdlssz,sxpjdlssz,axygglssz, bxygglssz,cxygglssz,sxzygglssz, divid);
	return false;
}

function initRealtimeDisperseCurve(axdyssz, bxdyssz,cxdyssz,sxpjdyssz,axdlssz, bxdlssz,cxdlssz,sxpjdlssz,axygglssz, bxygglssz,cxygglssz,sxzygglssz, divid) {
	Highcharts.setOptions({
        global: {
            useUTC: false
        }
		...
		chart: {
            ...
            events:{
            	load:function(){
            		...
            		var series03 = this.series[2];
//            		var series04 = this.series[3];
            		var series05 = this.series[3];
            		var series06 = this.series[4];
            		var series07 = this.series[5];
//            		var series08 = this.series[7];
            		//var series09 = this.series[8];
            		//var series10 = this.series[9];
            		//var series11 = this.series[10];
            		//var series12 = this.series[11];
            		realtimeInterval=setInterval(function(){
            			var tabPanel = Ext.getCmp("frame_center_ids");
            			var activeId = tabPanel.getActiveTab().id;
            			var leftOrg_Id = Ext.getCmp('leftOrg_Id');
            			if(activeId!="realtime_RealtimeMonitor"){
            				clearInterval(realtimeInterval);
            			}
                        if (!Ext.isEmpty(leftOrg_Id)) {
                            leftOrg_Id = leftOrg_Id.getValue();
                        }
                        var jh_obj = Ext.getCmp('RealtimeSelectedjh_Id');
                        var jh;
                        if (!Ext.isEmpty(jh_obj)) {
                        	jh = jh_obj.getValue();
                        }
            			//alert("获取数据");
            			Ext.Ajax.request({
            				url: context + '/realtimeMonitorDataManagerController/getNewRealtimeDisperseData',
            				method : "POST",
            				params : {
            					orgId: leftOrg_Id,
            	                jh:jh
            				...
            				success : function(response) {
            					...
            					if(list.length>0){
            						var getTime= Date.parse(list[0].cjsj.replace(/-/g, '/'));
                					var presentTime=Date.parse(Ext.getCmp("RealtimeDiaperseNewTime_Id").getValue().replace(/-/g, '/'));
                					if(getTime>presentTime){
                						Ext.getCmp("RealtimeDiaperseNewTime_Id").setValue(list[0].cjsj);
                						var voltageA=parseFloat(list[0].voltageA);
                    		            var voltageB=parseFloat(list[0].voltageB);
                    		            var voltageC=parseFloat(list[0].voltageC);
//                    		            var voltageAvg=parseFloat(list[0].voltageAvg);
                    		            
                    		            var currentA=parseFloat(list[0].currentA);
                    		            var currentB=parseFloat(list[0].currentB);
                    		            var currentC=parseFloat(list[0].currentC);
//                    		            var currentAvg=parseFloat(list[0].currentAvg);
                    		            
                    		            //var activePowerA=parseFloat(list[0].activePowerA);
                    		            //var activePowerB=parseFloat(list[0].activePowerB);
                    		            //var activePowerC=parseFloat(list[0].activePowerC);
                    		            //var activePowerSum=parseFloat(list[0].activePowerSum);
                    		            
                    		            var rw_g = Ext.getCmp("RealtimeDisperseDataGrid_Id");
                						if(isNotVal(rw_g)&&rw_g.store.data.length>0){
                							rw_g.store.getAt(0).set("value1",currentA);
                							rw_g.store.getAt(0).set("value2",voltageA);
                							rw_g.store.getAt(1).set("value1",currentB);
                							rw_g.store.getAt(1).set("value2",voltageB);
                							
                							rw_g.store.getAt(2).set("value1",currentC);
                							rw_g.store.getAt(2).set("value2",voltageC);
//                							rw_g.store.getAt(3).set("value1",currentC);
//                							rw_g.store.getAt(3).set("value2",currentAvg);
                							
                							//rw_g.store.getAt(4).set("value1",activePowerA);
                							//rw_g.store.getAt(4).set("value2",activePowerB);
                							//rw_g.store.getAt(5).set("value1",activePowerC);
                							//rw_g.store.getAt(5).set("value2",activePowerSum);
                							
                							for (i = 0; i < rw_g.store.getCount(); i ++) {  
                								  var record = rw_g.store.getAt(i);  
                								  if (record.dirty)  
                								     record.commit();  
                							} 
                						}
                    		            
                    		            series01.addPoint([getTime,currentA], true, false);
                    		            series02.addPoint([getTime,currentB], true, false);
                    		            series03.addPoint([getTime,currentC], true, false);
//                    		            series04.addPoint([getTime,currentAvg], true, false);
                    		            
                    		            series05.addPoint([getTime,voltageA], true, false);
                    		            series06.addPoint([getTime,voltageB], true, false);
                    		            series07.addPoint([getTime,voltageC], true, false);
//                    		            series08.addPoint([getTime,voltageAvg], true, false);
                    		            
                    		            //series09.addPoint([getTime,activePowerA], true, false);
                    		            //series10.addPoint([getTime,activePowerB], true, false);
                    		            //series11.addPoint([getTime,activePowerC], true, false);
                    		            //series12.addPoint([getTime,activePowerSum], true, false);
                					}
            					}
            				},
            				failure : function() {
 ...
 * 拼接单井信息曲线
 */
showElectricDiscreteAnalysisCurve = function(items, divid,jh) {
	var axdyssz = [],
		bxdyssz = [],
		cxdyssz=[],
		sxpjdyssz=[],
...
function initBalanceMonitorPieOrColChat(store,type) {
	var get_rawData = store.proxy.reader.rawData;
...
//		if(i==0){
//			pieDataStr+="{name:'"+datalist[i].gkmc+"',y:"+datalist[i].wellcount+",sliced: true,selected: true}";
//		}else{
...
//		}
	...
	var columnDivId="";
	if(type==1){
		tabPanelId="MonitorStatTorqueMeanSquareRootTabpanel_Id";
		piePanelId="MonitorStatTorqueMeanSquareRootPiePanel_Id";
		columnPanelId="MonitorStatTorqueMeanSquareRootColumnPanel_Id";
		pieDivId="MonitorStatTorqueMeanSquareRootPieDiv_Id";
		columnDivId="MonitorStatTorqueMeanSquareRootColumnDiv_Id";
	}else if(type==2){
		tabPanelId="MonitorStatTorqueMaxValueTabpanel_Id";
		piePanelId="MonitorStatTorqueMaxValuePiePanel_Id";
		columnPanelId="MonitorStatTorqueMaxValueColumnPanel_Id";
		pieDivId="MonitorStatTorqueMaxValuePieDiv_Id";
		columnDivId="MonitorStatTorqueMaxValueColumnDiv_Id";
	}else if(type==3){
		tabPanelId="MonitorStatTorqueAveragePowerTabpanel_Id";
		piePanelId="MonitorStatTorqueAveragePowerPiePanel_Id";
		columnPanelId="MonitorStatTorqueAveragePowerColumnPanel_Id";
		pieDivId="MonitorStatTorqueAveragePowerPieDiv_Id";
		columnDivId="MonitorStatTorqueAveragePowerColumnDiv_Id";
	}
	var tabPanel = Ext.getCmp(tabPanelId);
	var activeId = tabPanel.getActiveTab().id;
	if(activeId==piePanelId){
		ShowPieChart(pieDivId,"实时评价平衡状态统计图", "井数占", pieData);
	}else{
		ShowColumnChart(columnDivId,"实时评价平衡状态统计图", categories, colunmData);
	}
}

function ShowColumnChart(divid,title_, categories_, values_) {
	$('#'+divid).highcharts({
		chart : {
			type : 'column'
		},
...
function ShowPieChart(divid,title_, name_, data) {
	$('#'+divid).highcharts({
		chart : {
			plotBackgroundColor : null,
			plotBorderWidth : null,
...
//							alert("工况："+e.point.name+" 井数："+e.point.y);
							Ext.getCmp("TorqueMaxValueBalanceMonitorSelectedGkmc_Id").setValue(e.point.name);
						}else{
							Ext.getCmp("TorqueMaxValueBalanceMonitorSelectedGkmc_Id").setValue("");
						}
...
function ShowTotalBalanceStatPieChart(divid,title_, name_, data) {
	$('#'+divid).highcharts({
		chart : {
			plotBackgroundColor : null,
			plotBorderWidth : null,
			...
			pie : {
				...
				events: {
					click: function(e) {
						if(!e.point.selected){//如果没被选中
//							alert("工况："+e.point.name+" 井数："+e.point.y);
							Ext.getCmp("TorqueMaxValueBalanceTotalSelectedGkmc_Id").setValue(e.point.name);
						}else{
							Ext.getCmp("TorqueMaxValueBalanceTotalSelectedGkmc_Id").setValue("");
						}
						Ext.getCmp("TorqueMaxValueTotaljh_Id").setValue("");
                		Ext.getCmp("TorqueMaxValueTotaljh_Id").setRawValue("");
...
function ShowCycleBalanceStatPieChart(divid,title_, name_, data) {
	$('#'+divid).highcharts({
		chart : {
			plotBackgroundColor : null,
			plotBorderWidth : null,
			...
			pie : {
				...
				events: {
					click: function(e) {
						if(!e.point.selected){//如果没被选中
//							alert("工况："+e.point.name+" 井数："+e.point.y);
							Ext.getCmp("TorqueMaxValueBalanceCycleSelectedGkmc_Id").setValue(e.point.name);
						}else{
							Ext.getCmp("TorqueMaxValueBalanceCycleSelectedGkmc_Id").setValue("");
						}
						Ext.getCmp("TorqueMaxValueCyclejh_Id").setValue("");
                		Ext.getCmp("TorqueMaxValueCyclejh_Id").setRawValue("");
...
function initPSDiagramOverlayChart(series, title,ytext, jh, cjsj, divid) {
	mychart = new Highcharts.Chart({
				chart: {                                                                             
		            type: 'scatter',      // 散点图   
		            renderTo : divid,
