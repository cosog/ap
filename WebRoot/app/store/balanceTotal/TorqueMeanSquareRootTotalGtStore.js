Ext.define('AP.store.balanceTotal.TorqueMeanSquareRootTotalGtStore', {
    extend: 'Ext.data.Store',
    id: 'TorqueMeanSquareRootTotalGtStore_Id',
    alias: 'widget.torqueMeanSquareRootTotalGtStore',
...
//            Ext.getCmp("TorqueMeanSquareRootTotalEndDate_Id").setValue(get_rawData.endDate);
            var totals = get_rawData.totals; // 总记录数
            var totalPages = get_rawData.totalPages; // 总页数
            Ext.getCmp("TorqueMeanSquareRootTotalGtTotalPages_Id").setValue("");
            Ext.getCmp("TorqueMeanSquareRootTotalGtTotalPages_Id").setValue(totalPages);
            ...
            $("#TorqueMeanSquareRootTotalGtDiv_Id").html(''); // 将html内容清空
            var htmlResult = '';
            var divId = '';
            ...
            if (totals > 0) {
                ...
                Ext.Array.each(graphicalList, function (name, index, countriesItSelf) {
                    divId = 'TorqueMeanSquareRootTotalGt' + graphicalList[index].id;
                ...
                Ext.Array.each(graphicalList, function (name, index, countriesItSelf) {
                    divId = 'TorqueMeanSquareRootTotalGt' + graphicalList[index].id;
                    showBalanceCycleSurfaceCardChart(graphicalList[index], divId,Ext.getCmp("TorqueMeanSquareRootTotalAnalysisiDataGrid_Id").getSelectionModel().getSelection()[0].data.jh); // 调用画功图的函数，功图列表
                });
            }
        },
...
//            onStoreSizeChange(v, o, "TorqueMeanSquareRootTotalGtCount_Id"); //获取总记录数
        }
    }
});