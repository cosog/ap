...
//	var deviceCount=getCalculateTypeDeviceCount(orgId,deviceType,1);
	...
	if(projectTabConfig.DeviceHistoryQuery.NumStatusStatPie==false){
		tabPanel.remove(Ext.getCmp("HistoryQueryNumStatusStatGraphPanel_Id"));
		if(statTabActiveId=="HistoryQueryNumStatusStatGraphPanel_Id"){
			tabChange=true;
		}
	}else{
		var HistoryQueryNumStatusStatGraphPanel = tabPanel.getComponent("HistoryQueryNumStatusStatGraphPanel_Id");
		if(HistoryQueryNumStatusStatGraphPanel==undefined){
			tabPanel.insert(3,historyStatTabItems[3]);
		}
	}
	
	if(projectTabConfig.DeviceHistoryQuery.FESDiagramStatPie==false 
			&& projectTabConfig.DeviceHistoryQuery.CommStatusStatPie==false 
			&& projectTabConfig.DeviceHistoryQuery.RunStatusStatPie==false
			&& projectTabConfig.DeviceHistoryQuery.NumStatusStatPie==false
			){
		tabPanel.hide();
		...
		if(tabPanel.getActiveTab()==undefined){
			...
			if(!tabChange){
				if(statTabActiveId=="HistoryQueryFESdiagramResultStatGraphPanel_Id"){
		 		...
		 		}else if(statTabActiveId=="HistoryQueryNumStatusStatGraphPanel_Id"){
...
function exportHistoryQueryDeviceListExcel(orgId,deviceType,deviceName,dictDeviceType,FESdiagramResultStatValue,commStatusStatValue,runStatusStatValue,numStatusStatValue,deviceTypeStatValue,fileName,title,columnStr) {
...
function loadAndInitHistoryQueryNumStatusStat(all){
	var orgId = Ext.getCmp('leftOrg_Id').getValue();
	var deviceType=getDeviceTypeFromTabId("HistoryQueryRootTabPanel");
	var activeId = Ext.getCmp("HistoryQueryRootTabPanel").getActiveTab().id;
	var panelId='HistoryQueryNumStatusStatGraphPanel_Id';
	if(all){
		Ext.getCmp("HistoryQueryStatSelectFESdiagramResult_Id").setValue('');
		Ext.getCmp("HistoryQueryStatSelectCommStatus_Id").setValue('');
		Ext.getCmp("HistoryQueryStatSelectRunStatus_Id").setValue('');
	...
	Ext.Ajax.request({
		...
		success:function(response) {
			...
			var result =  Ext.JSON.decode(response.responseText);
			Ext.getCmp("AlarmShowStyle_Id").setValue(JSON.stringify(result.AlarmShowStyle));
			initHistoryQueryNumStatusStatPieOrColChat(result);
		},
		failure:function(){
			if(isNotVal(Ext.getCmp(panelId))){
				Ext.getCmp(panelId).getEl().unmask();
...
/**
 * 带回调的图表渲染函数
 * @param {Object} diagram - 功图数据
 * @param {String} divId - 容器ID
 * @param {Function} callback - 渲染完成回调
 */
function renderChartWithCallback(diagram, divId, callback) {
    showSurfaceCard(diagram, divId);
...
/**
 * 逐个渲染图表
 * @param {Array} renderQueue - 渲染队列 [{diagram: ..., divId: ...}]
 * @param {Function} onComplete - 全部完成回调
 */
function renderChartsSequentially(renderQueue, onComplete) {
    ...
    function renderNext() {
        ...
        if (completedCount >= totalCount) {
            // 所有图表渲染完成
            console.log("所有图表渲染完成！成功: " + (totalCount - failedCount) + 
        ...
        renderChartWithCallback(item.diagram, item.divId, function(chart) {
            if (chart) {
                console.log("图表 " + (index + 1) + "/" + totalCount + " 渲染成功: " + item.divId);
            } else {
                failedCount++;
                console.warn("图表 " + (index + 1) + "/" + totalCount + " 渲染失败: " + item.divId);
            }
...
loadSurfaceCardList = function (page) {
    diagramTiledPage = page;
    Ext.getCmp("HistoryDiagramTabPanel").mask(loginUserLanguageResource.loadingData);
    var start = (page - 1) * defaultGraghSize;
    if (page == 1) {
    ...
    Ext.Ajax.request({
        ...
        success: function (response) {
            ...
            Ext.getCmp("HistoryDiagramTabPanel").unmask(loginUserLanguageResource.loadingData);
            var get_rawData = Ext.decode(response.responseText);
            var diagramList = get_rawData.list;

            var totals = get_rawData.totals;
            var totalShow = get_rawData.totalShow;
            var totalPages = get_rawData.totalPages;
            Ext.getCmp("SurfaceCardTotalPages_Id").setValue(totalPages);
            updateTotalRecords(totalShow, "HistoryFESDiagramTotalCount_Id");

            var startDateCmp = Ext.getCmp('HistoryFSDiagramQueryStartDate_Id');
            if (startDateCmp.rawValue == '' || null == startDateCmp.rawValue) {
                startDateCmp.setValue(get_rawData.start_date.split(' ')[0]);
                Ext.getCmp('HistoryFSDiagramQueryStartTime_Hour_Id').setValue(get_rawData.start_date.split(' ')[1].split(':')[0]);
                Ext.getCmp('HistoryFSDiagramQueryStartTime_Minute_Id').setValue(get_rawData.start_date.split(' ')[1].split(':')[1]);
            }
            var endDateCmp = Ext.getCmp('HistoryFSDiagramQueryEndDate_Id');
            if (endDateCmp.rawValue == '' || null == endDateCmp.rawValue) {
                endDateCmp.setValue(get_rawData.end_date.split(' ')[0]);
                Ext.getCmp('HistoryFSDiagramQueryEndTime_Hour_Id').setValue(get_rawData.end_date.split(' ')[1].split(':')[0]);
                Ext.getCmp('HistoryFSDiagramQueryEndTime_Minute_Id').setValue(get_rawData.end_date.split(' ')[1].split(':')[1]);
            }
            ...
            var panelHeight = HistoryDiagramTabPanel.getHeight();
            var panelWidth = HistoryDiagramTabPanel.getWidth();
            var scrollWidth = getScrollWidth();

            var scroller = HistoryDiagramTabPanel.getScrollable();
            var scrollSize = scroller.getSize().y;
            var clientSize = scroller.getClientSize().y;
            var hasVerticalScroll = scrollSize > clientSize;
            ...
            if (diagramList && diagramList.length > 0) {
                ...
                Ext.Array.each(diagramList, function (name, index, countriesItSelf) {
                    var diagramId = diagramList[index].id;
                    var divId = 'DiagramTiled_FSDiagram_Id_' + diagramId;
                    ...
                    renderQueue.push({
                        diagram: diagramList[index],
                        divId: divId
                    });
                });
                
                console.log("创建 " + renderQueue.length + " 个图表容器完成，开始逐个渲染（间隔10ms）");
                
                // 逐个渲染图表
                renderChartsSequentially(renderQueue, function() {
                    console.log("第 " + page + " 页所有图表渲染完成");
                });
            } else {
                console.log("没有功图数据需要渲染");
            }
...
function updateDeviceHistoryQueryData(record){
	var deviceType=getDeviceTypeFromTabId("HistoryQueryRootTabPanel");
	var deviceName='';
	var deviceId=0;
	...
	if(!showHistoryQueryCenterTabPanel){
		tabPanel.hide();
	}else{
		if(tabPanel.isHidden() ){
			tabPanel.show();
