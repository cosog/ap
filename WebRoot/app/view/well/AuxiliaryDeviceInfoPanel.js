    		...
    		items: [{
        		...
        		items: [{
                	...
                	tbar: [{
                        ...
                        labelWidth: getLabelWidth(loginUserLanguageResource.stroke,loginUserLanguage),
                        width: getLabelWidth(loginUserLanguageResource.stroke,loginUserLanguage)+110,
                        labelAlign: 'left',
                        triggerAction: 'all',
                        displayField: "boxval",
                        valueField: "boxkey",
                    ...
                    success: function (response) {
                        ...
                        if (rdata.success) {
                        	...
                        	if(rdata.collisionCount>0){//数据冲突
                        		saveInfo=loginUserLanguageResource.savedSuccessfully+":"+rdata.successCount+','+loginUserLanguageResourceFirstLower.saveFailed+':<font color="red">'+rdata.collisionCount+'</font>';
	...
	Ext.Ajax.request({
		...
		success:function(response) {
			...
			if(auxiliaryDeviceDetailsHandsontableHelper==null || auxiliaryDeviceDetailsHandsontableHelper.hot==undefined){
				...
				var colHeaders="['"+loginUserLanguageResource.idx+"','"+loginUserLanguageResource.variable+"','"+loginUserLanguageResource.value+"','"+loginUserLanguageResource.unit+"','itemCode']";
...
//	                        }
	                ...
	                afterOnCellMouseOver: function(event, coords, TD){
	                	if(coords.col>=0 && coords.row>=0 && auxiliaryDeviceDetailsHandsontableHelper!=null&&auxiliaryDeviceDetailsHandsontableHelper.hot!=''&&auxiliaryDeviceDetailsHandsontableHelper.hot!=undefined && auxiliaryDeviceDetailsHandsontableHelper.hot.getDataAtCell!=undefined){
	                		var rawValue=auxiliaryDeviceDetailsHandsontableHelper.hot.getDataAtCell(coords.row,coords.col);
	                		if(isNotVal(rawValue)){
	            				var showValue=rawValue;
	        					var rowChar=90;
	        					var maxWidth=rowChar*10;
...
//	                        }
	                ...
	                afterOnCellMouseOver: function(event, coords, TD){
	                	if(coords.col>=0 && coords.row>=0 && pumpingUnitPRTFHandsontableHelper!=null&&pumpingUnitPRTFHandsontableHelper.hot!=''&&pumpingUnitPRTFHandsontableHelper.hot!=undefined && pumpingUnitPRTFHandsontableHelper.hot.getDataAtCell!=undefined){
	                		var rawValue=pumpingUnitPRTFHandsontableHelper.hot.getDataAtCell(coords.row,coords.col);
	                		if(isNotVal(rawValue)){
	            				var showValue=rawValue;
	        					var rowChar=90;
	        					var maxWidth=rowChar*10;
