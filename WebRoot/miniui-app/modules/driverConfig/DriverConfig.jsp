...
<body>
    ...
    <script>
     ...
     function onAcqProtocolTreeLoad(e) {
    	    ...
    	    if (_selectedAcqUnitProtocolCode) {
    	        // 深度遍历查找
    	        function findNode(node) {
    	            if (node.code === _selectedAcqUnitProtocolCode && node.classes === 1) {
    	                targetNode = node;
    	                return true;
    	            }
    	            if (node.children) {
     ...
     var _selectedAcqUnitProtocolCode = null;
  	 // 单元列表树选中状态
     ...
     function onAcqProtocolTreeSelect(e) {
    	    var node = e.node;
    	    currentAcqProtocolNode = node;
    	    if (node && node.classes === 1) {
    	    	_selectedAcqUnitProtocolCode = node.code;
  ...
  // 采控单元 - 单元列表事件
  // ================================================================
  var currentAcqProtocolNode = null;
  ...
  function onAcqUnitListBeforeLoad(e) {
	    var params = e.params || {};
	    if (currentAcqProtocolNode) {
	    	if(currentAcqProtocolNode.classes==1){
	    		params.protocol = currentAcqProtocolNode.code;
	    	}else{
	    		var protocolList=[];
	    		if(isNotVal(currentAcqProtocolNode.children)){
    				for(var i=0;i<currentAcqProtocolNode.children.length;i++){
    					protocolList.push(currentAcqProtocolNode.children[i].code);
    				}
    			}
  ...
  function onAcqUnitListLoad(e) {
	    ...
	    if (!targetNode && _selectedAcqUnitId) {
	        targetNode = tree.getNode(_selectedAcqUnitId);
	    }
...
//采控单元列表选中事件
	var currentAcqUnitNode=null;
  function onAcqUnitListSelect(e) {
    var node = e.node;
    if (!node) return;
    currentAcqUnitNode = node;
...
//Tab 切换事件
  function onAcqUnitDetailTabChanged(e) {
      if (!currentAcqUnitNode) return;
      var tabs = e.sender;
      var active = tabs.getActiveTab();
      if (!active) return;
      var tabName = active.name; // 'props' 或 'config'
      if (tabName === 'props') {
          loadAcqUnitProperties(currentAcqUnitNode);
      } else if (tabName === 'config') {
          loadAcqUnitConfig(currentAcqUnitNode);
      }
  }
       ...
       helper.createTable = function (data) {
           ...
           helper.hot = new Handsontable(hotElement, {
               ...
               cells: function (row, col, prop) {
                   ...
                   var protocolConfigModuleEditFlag = (typeof editFlag !== 'undefined' && editFlag) ? 1 : 0;
                   if (protocolConfigModuleEditFlag == 1) {
                       var selectedItem = currentAcqUnitNode; // 当前选中的节点
...
function addAcquisitionUnitInfo() {
    var deviceTree = mini.get('deviceTypeTree');
    if (!deviceTree) {
        mini.alert(_loginUserLanguageResource.selectDeviceType || '请先选择设备类型');
        return;
    }
    ...
    mini.open({
        title: _loginUserLanguageResource.addAcqUnit || '添加采集单元',
