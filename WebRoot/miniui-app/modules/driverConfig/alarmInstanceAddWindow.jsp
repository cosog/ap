...
<div style="padding:10px;">
    <form id="instanceForm" class="mini-form">
        <table class="form-table">
            <!-- 报警单元（树选择，禁止选中父节点） -->
            <tr>
                <td class="label"><span style="color:red;">*</span><span id="lblAlarmUnit"></span>：</td>
                <td>
                    <input id="alarmUnitTreeSelect" class="mini-treeselect"
                           style="width:100%;"
                           multiSelect="false"
                           ...
                           showTreeIcon="true"
                           required="true"
                           onbeforeload="onAlarmUnitTreeBeforeLoad"
                           onload="onAlarmUnitTreeLoad"
                           onbeforenodeselect="onAlarmUnitTreeBeforeNodeSelect"
                           onvaluechanged="onAlarmUnitValueChanged" />
...
<script>
    ...
    function processSetData() {
        ...
        // 加载报警单元树
        var treeSelect = mini.get('alarmUnitTreeSelect');
        if (treeSelect) {
            treeSelect.setUrl(context + '/acquisitionUnitManagerController/modbusProtocolAndAlarmUnitTreeData');
            treeSelect.load();
        }
    }
    ...
    // 2. 报警单元树事件
    // ================================================================
    function onAlarmUnitTreeBeforeLoad(e) {
        var params = e.params || {};
    ...
    // 只允许选中报警单元节点（classes === 2）
    function onAlarmUnitTreeBeforeNodeSelect(e) {
        if (!e.node) {
            e.cancel = true;
            return;
        }
    ...
    function onAlarmUnitTreeLoad(e) {
        // 预留
    }

    function onAlarmUnitValueChanged(e) {
        var treeSelect = mini.get('alarmUnitTreeSelect');
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (parseInt(resp.msg) === 1) {
                    mini.confirm(
                        '<font color="red">' + (_loginUserLanguageResource.alarmInstanceExist || '实例已存在') +
    ...
    function onSave() {
        ...
        var unitId = mini.get('unitId').getValue();
        if (!unitId) {
            mini.alert('<font color="red">' + (_loginUserLanguageResource.selectAlarmUnit || '请选择报警单元') + '</font>');
            return;
        }

        if (!_instanceNameValid) {
            mini.alert('<font color="red">' + (_loginUserLanguageResource.alarmInstanceExist || '实例名称无效') + '，请重新输入</font>');
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                'protocolAlarmInstance.AlarmUnitId': unitId,
                'protocolAlarmInstance.name': instanceName,
                'protocolAlarmInstance.sort': sort
            },
            dataType: 'json',
            success: function (resp) {
    ...
    function initI18n() {
        document.title = _loginUserLanguageResource.addAlarmInstance;
        ...
        document.getElementById('lblAlarmUnit').textContent = _loginUserLanguageResource.alarmUnit;
        ...
        var treeSelect = mini.get('alarmUnitTreeSelect');
        if (treeSelect) {
            treeSelect.setEmptyText(_loginUserLanguageResource.selectAlarmUnit + '...');
        }
    }
</script>
</body>
