...
<script>
    ...
    function setData(data) {
        ...
        if (tree) {
            tree.load(context + '/acquisitionUnitManagerController/exportAlarmUnitTreeData');
        }
    }
    ...
    function onTreeDrawnode(e) {
        // 只对报警单元（classes === 3）显示复选框
        if (e.node.classes != 3) e.showCheckBox = false;
    }

    function initI18n() {
        document.title = _loginUserLanguageResource.exportAlarmUnit;
    ...
    function onExport() {
        ...
        var key = 'exportProtocolAlarmUnitData_' + timestamp;
        var url = context + '/acquisitionUnitManagerController/exportProtocolAlarmUnitData?key=' + key + '&unitList=' + unitList.join(',');
