...
<div class="main-container">
    ...
    <div class="mini-toolbar">
        ...
        <form id="uploadForm"
              action="<%=context%>/acquisitionUnitManagerController/uploadImportedAcqInstanceFile"
...
<script>
    ...
    // ================================================================
    function initI18n() {
        document.title = _loginUserLanguageResource.importAcqInstance;
    ...
    function setData(data) {
        ...
        //var tree = mini.get('instanceTree');
        //if (tree) {
        //    tree.load(context + '/acquisitionUnitManagerController/getUploadedAcqInstanceTreeData');
        //}
    }
    ...
    function onTreeLoad(e) {
        ...
        // ★ 设置列（与 ExtJS 一致：预导入实例 / 碰撞信息 / 保存）
        tree.setColumns([
            {
                field: 'text',
                name: 'taskname',
                header: _loginUserLanguageResource.importAcqInstance,
                headerAlign: 'left',
                align: 'left',
                width: '50%'
            },
    ...
    function onTreeDrawcell(e) {
        ...
        if (field === 'msg' && record.classes == 1) {
            ...
            if (record.saveSign != 2) {
                ...
                e.cellHtml = '<a href="javascript:void(0)" onclick="saveSingleImportedAcqInstance(\''
    ...
    // ================================================================
    // 5. 单个保存
    //    参考原 ExtJS: saveSingelImportedAcqInstance()
    // ================================================================
    function saveSingleImportedAcqInstance(instanceName, unitName, protocolName,protocolDeviceType, saveSign, msg) {
        ...
        function doSave() {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    // 刷新父页面实例列表树
                    if (window.parent && window.parent.refreshAcqInstanceList) {
                        window.parent.refreshAcqInstanceList();
                    }
                },
                error: function () {
                    mini.unmask(document.body);
    ...
    // ================================================================
    // 6. 全部保存
    //    参考原 ExtJS: saveAllImportedAcqInstance()
    // ================================================================
    function onSaveAll() {
        var tree = mini.get('instanceTree');
        var root = tree.getRootNode();
        ...
        function collect(node) {
            if (node.classes === 1) {
                if (node.saveSign === 1) {
                    overlayCount++;
                } else if (node.saveSign === 2) {
                    collisionCount++;
                }
                if (node.saveSign !== 2) {
                    instanceNames.push(node.text);
                }
            }
            if (node.children) {
