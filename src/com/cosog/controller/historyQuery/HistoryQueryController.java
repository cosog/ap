		...
		try {
			if(StringManagerUtils.isNotNull(deviceId)&&!StringManagerUtils.isNotNull(endDate)){
				String sql = " select to_char(t.fesdiagramacqtime+1/(24*60),'yyyy-mm-dd hh24:mi:ss') from "+tableName+" t "
						+ " where t.id=  (select max(t2.id) from "+tableName+" t2 where t2.fesdiagramacqtime is not null and t2.deviceId= "+deviceId+")";
				List list = this.service.reportDateJssj(sql);
				if (list.size() > 0 &&list.get(0)!=null&&!list.get(0).toString().equals("null")) {
					endDate = list.get(0).toString();
				} else {
...
//			StringManagerUtils.printLog(StringManagerUtils.getCurrentTime("yyyy-MM-dd HH:mm:ss")+":设备"+deviceId+"功图平铺数据查询总耗时:"+StringManagerUtils.getTimeDiff(t1, t2),0);
	...
	public String getFESDiagramOverlayData() throws IOException {
		...
		try {
			if(StringManagerUtils.isNotNull(deviceId)&&!StringManagerUtils.isNotNull(endDate)){
				String sql = " select to_char(t.fesdiagramacqtime+1/(24*60),'yyyy-mm-dd hh24:mi:ss') from "+tableName+" t "
						+ " where t.id=  (select max(t2.id) from "+tableName+" t2 where "
								+ " t2.fesdiagramacqtime is not null and t2.deviceId= "+deviceId+")";
				List list = this.service.reportDateJssj(sql);
				if (list.size() > 0 &&list.get(0)!=null&&!list.get(0).toString().equals("null")) {
					endDate = list.get(0).toString();
				} else {
	...
	public String getDeviceResultStatusStatData() throws Exception {
		...
		if(StringManagerUtils.isNotNull(deviceId)&&!StringManagerUtils.isNotNull(endDate)){
			String sql = " select to_char(t.fesdiagramacqtime+1/(24*60),'yyyy-mm-dd hh24:mi:ss') from "+tableName+" t "
					+ " where t.id=  (select max(t2.id) from "+tableName+" t2 "
							+ " where t2.fesdiagramacqtime is not null"
//							+ " and t2.resultstatus=1 "
