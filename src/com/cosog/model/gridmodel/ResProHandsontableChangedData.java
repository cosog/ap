package com.cosog.model.gridmodel;

import java.util.List;

public class ResProHandsontableChangedData {
	
	private List<String> delidslist;

    private List<Updatelist> updatelist;

    private List<Insertlist> insertlist;

    public void setDelidslist(List<String> delidslist){
        this.delidslist = delidslist;
    }
    public List<String> getDelidslist(){
        return this.delidslist;
    }
    public void setUpdatelist(List<Updatelist> updatelist){
        this.updatelist = updatelist;
    }
    public List<Updatelist> getUpdatelist(){
        return this.updatelist;
    }
    public void setInsertlist(List<Insertlist> insertlist){
        this.insertlist = insertlist;
    }
    public List<Insertlist> getInsertlist(){
        return this.insertlist;
    }
    
	public static class Updatelist
	{
	    private String jlbh;

	    private String yqcbh;

	    private String resName;

	    private String yymd;

	    private String smd;

	    private String trqxdmd;

	    private String bhyl;

	    private String yqcyl;

	    private String yqczbsd;

	    private String yqczbwd;

	    public void setJlbh(String jlbh){
	        this.jlbh = jlbh;
	    }
	    public String getJlbh(){
	        return this.jlbh;
	    }
	    public void setYqcbh(String yqcbh){
	        this.yqcbh = yqcbh;
	    }
	    public String getYqcbh(){
	        return this.yqcbh;
	    }
	    public void setResName(String resName){
	        this.resName = resName;
	    }
	    public String getResName(){
	        return this.resName;
	    }
	    public void setYymd(String yymd){
	        this.yymd = yymd;
	    }
	    public String getYymd(){
	        return this.yymd;
	    }
	    public void setSmd(String smd){
	        this.smd = smd;
	    }
	    public String getSmd(){
	        return this.smd;
	    }
	    public void setTrqxdmd(String trqxdmd){
	        this.trqxdmd = trqxdmd;
	    }
	    public String getTrqxdmd(){
	        return this.trqxdmd;
	    }
	    public void setBhyl(String bhyl){
	        this.bhyl = bhyl;
	    }
	    public String getBhyl(){
	        return this.bhyl;
	    }
	    public void setYqcyl(String yqcyl){
	        this.yqcyl = yqcyl;
	    }
	    public String getYqcyl(){
	        return this.yqcyl;
	    }
	    public void setYqczbsd(String yqczbsd){
	        this.yqczbsd = yqczbsd;
	    }
	    public String getYqczbsd(){
	        return this.yqczbsd;
	    }
	    public void setYqczbwd(String yqczbwd){
	        this.yqczbwd = yqczbwd;
	    }
	    public String getYqczbwd(){
	        return this.yqczbwd;
	    }
	}
	
	public static class Insertlist
	{
	    private String jlbh;

	    private String yqcbh;

	    private String resName;

	    private String yymd;

	    private String smd;

	    private String trqxdmd;

	    private String bhyl;

	    private String yqcyl;

	    private String yqczbsd;

	    private String yqczbwd;

	    public void setJlbh(String jlbh){
	        this.jlbh = jlbh;
	    }
	    public String getJlbh(){
	        return this.jlbh;
	    }
	    public void setYqcbh(String yqcbh){
	        this.yqcbh = yqcbh;
	    }
	    public String getYqcbh(){
	        return this.yqcbh;
	    }
	    public void setResName(String resName){
	        this.resName = resName;
	    }
	    public String getResName(){
	        return this.resName;
	    }
	    public void setYymd(String yymd){
	        this.yymd = yymd;
	    }
	    public String getYymd(){
	        return this.yymd;
	    }
	    public void setSmd(String smd){
	        this.smd = smd;
	    }
	    public String getSmd(){
	        return this.smd;
	    }
	    public void setTrqxdmd(String trqxdmd){
	        this.trqxdmd = trqxdmd;
	    }
	    public String getTrqxdmd(){
	        return this.trqxdmd;
	    }
	    public void setBhyl(String bhyl){
	        this.bhyl = bhyl;
	    }
	    public String getBhyl(){
	        return this.bhyl;
	    }
	    public void setYqcyl(String yqcyl){
	        this.yqcyl = yqcyl;
	    }
	    public String getYqcyl(){
	        return this.yqcyl;
	    }
	    public void setYqczbsd(String yqczbsd){
	        this.yqczbsd = yqczbsd;
	    }
	    public String getYqczbsd(){
	        return this.yqczbsd;
	    }
	    public void setYqczbwd(String yqczbwd){
	        this.yqczbwd = yqczbwd;
	    }
	    public String getYqczbwd(){
	        return this.yqczbwd;
	    }
	}
}
