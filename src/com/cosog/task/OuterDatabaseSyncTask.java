...
//	@Scheduled(fixedRate = 1000*60*60*24*365*100)
	public static void RPCWellDataSync(){
		...
		if(dataSourceConfig!=null && dataSourceConfig.isEnable()){
			boolean sign=false;
			do{
				String sql="select t.wellname,to_char(t2.acqtime,'yyyy-mm-dd hh24:mi:ss') as acqtime,to_char(t2.fesdiagramacqtime,'yyyy-mm-dd hh24:mi:ss') as fesdiagramacqtime from tbl_rpcdevice t,tbl_rpcacqdata_latest t2 where t.id=t2.wellid and t.status=1";
						...
						if(dataSourceConfig.getDiagramTable().getEnable()){
							...
							do{
								sql="select "+wellNameColumn+","
										...
										+ " where t.wellname='"+dataSourceConfig.getDiagramTable().getColumns().getWellName().getColumn()+"'";
