...
//	        }
            ...
            try{
            	String sql="SELECT"
            			...
            			+ " ROUND(SUM(CASE WHEN NVL(df.maxbytes, 0) = 0 THEN df.bytes ELSE df.maxbytes END)/1024/1024, 2) AS max_allowed_mb,"
            			...
            			+ " ROUND((SUM(df.bytes) - SUM(NVL(fs.free_bytes, 0))) / SUM(CASE WHEN NVL(df.maxbytes, 0) = 0 THEN df.bytes ELSE df.maxbytes END) * 100, 2) AS pct_used_of_max"
