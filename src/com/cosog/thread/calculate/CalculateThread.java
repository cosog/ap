...
public class CalculateThread extends Thread{
	...
	public void run(){
		StringManagerUtils.printLog("线程"+threadId+"开始计算"+(calculateType==1?"抽油机井":"螺杆泵井")+"编号"+wellNo+"井",0);
		...
		if(calculateType==1){
			...
			String sql="select t2.deviceName,decode(t2.applicationscenarios,0,'cbm','oil') as applicationscenarios,"
					...
					+ " left outer join viw_device t2 on t.deviceId=t2.id"
				...
				try{
					...
					if(calculateRequestData!=null){
						...
						if(calculateResponseData==null){
							StringManagerUtils.printLog("记录:"+recordId+"计算无数据返回",0);
						}
		...
		StringManagerUtils.printLog("计算单条记录数:"+count+",汇总记录数:"+totalCount+",共用时:"+(endTime-startTime)+"ms",0);
	}
