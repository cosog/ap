...
public class DiagnosisAnalysisOnlyController extends BaseController {
	...
	public String getProductionWellRTAnalysisWellList() throws Exception {
		...
		if(StringManagerUtils.isNotNull(wellName)&&!StringManagerUtils.isNotNull(endDate)){
			String sql = " select to_char(max(t.acquisitionTime),'yyyy-mm-dd') from t_indicatordiagram t where t.wellId=( select t2.id from t_wellinformation t2 where t2.wellName='"+wellName+"' ) ";
	...
	public String getDiagnosisDataCurveData() throws Exception {
		...
		if(!StringManagerUtils.isNotNull(endDate)){
			String sql = " select to_char(max(t.gtcjsj),'yyyy-mm-dd') from v_analysishistory t  where wellName= '"+wellName+"' ";
			List list = this.service.reportDateJssj(sql);
			if (list.size() > 0 &&list.get(0)!=null&&!list.get(0).toString().equals("null")) {
				endDate = list.get(0).toString();
			} else {
