...
//		String columns=service.showTableHeadersColumns("acquisitionUnit");
	...
	public String getDriverConfigData(){
		...
		for(Entry<Integer, Object> entry:equipmentDriveSortMap.entrySet()){
			...
			if(!driveConfig.getProtocolCode().toUpperCase().contains("KAFKA") && !driveConfig.getProtocolCode().toUpperCase().contains("MQTT")){
				...
				driverConfigData.append("]");
				result_json.append("{\"id\":1,\"driverName\":\""+driveConfig.getProtocolName()+"\",\"dataConfig\":"+driverConfigData.toString()+"},");
			}
	...
	public String getTcpServerConfigData(){
		...
		result_json.append("{\"id\":2,\"item\":\"心跳前缀\",\"value\":\""+tcpServerConfig.getHeartbeatPrefix()+"\"},");
		result_json.append("{\"id\":3,\"item\":\"心跳后缀\",\"value\":\""+tcpServerConfig.getHeartbeatSuffix()+"\"}");
		result_json.append("]}");
		return result_json.toString();
	}
