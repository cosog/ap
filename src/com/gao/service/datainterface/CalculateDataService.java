...
public class CalculateDataService<T> extends BaseService<T> {
	...
	public String getFESdiagramCalculateRequestData(AcquisitionGroupResolutionData acquisitionGroupResolutionData) throws SQLException, IOException, ParseException{
		Gson gson = new Gson();
		...
		try{
			String prodDataSql="select t.id,"
					...
					+ " and t3.wellName='"+acquisitionGroupResolutionData.getWellName()+"'";
			String rpcInformationSql="select t2.manufacturer,t2.model,decode(t2.crankrotationdirection,'顺时针','Clockwise','Anticlockwise'),"
					+ " t2.offsetangleofcrank,t2.crankgravityradius,"
					+ " t2.singlecrankweight,t2.singlecrankpinweight,"
					+ " t2.structuralunbalance,"
					...
					+ " from  tbl_wellinformation t,tbl_rpcinformation t2"
					+ " where t.id=t2.wellid"
					+ " and t.wellname='"+acquisitionGroupResolutionData.getWellName()+"'";
			String wellboretrajectorySql="select t2.measuringdepth,t2.deviationangle,t2.azimuthangle"
					+ " from  tbl_wellinformation t,tbl_wellboretrajectory t2"
					+ " where t.id=t2.wellid"
					+ " and t.wellname='"+acquisitionGroupResolutionData.getWellName()+"'";
			...
			if(prodDataList.size()>0){
				...
				acquisitionGroupResolutionData.setProdDataId(StringManagerUtils.stringToInteger(object[0]+""));
				acquisitionGroupResolutionData.setWellId(StringManagerUtils.stringToInteger(object[object.length-1]+""));
				calculateRequestData.setWellName(acquisitionGroupResolutionData.getWellName());
...
//				calculateRequestData.setWellboreTrajectory(new RPCCalculateRequestData.WellboreTrajectory());
	...
	public boolean saveFESDiagramAndCalculateData(AcquisitionGroupResolutionData acquisitionGroupResolutionData,RPCCalculateResponseData calculateResponseData) throws SQLException, ParseException{
		boolean result=false;
		result=this.getBaseDao().saveFESDiagramAndCalculateData(acquisitionGroupResolutionData,calculateResponseData);
		return result;
	}
...
//								runRange=wellObj[12]+"";
	...
	public List<String> getFSDiagramDailyCalculationRequestData(String tatalDate,String wellId,String endAcqTime) throws ParseException, SQLException, IOException{
		String date=StringManagerUtils.addDay(StringManagerUtils.stringToDate(tatalDate),-1);
		StringBuffer dataSbf=null;
		List<String> requestDataList=new ArrayList<String>();
		String singleCalculateResuleSql="select t007.wellname,to_char(t.acqTime,'yyyy-mm-dd hh24:mi:ss') as acqTime,t.workingconditioncode,"
											...
											+ "t.deltaRadius,"
											+" prod.tubingpressure,prod.casingpressure,prod.wellheadfluidtemperature,prod.productiongasoilratio,"
											+ "dis.commstatus,dis.commtimeefficiency,dis.commtime,dis.commrange,"
											+ "dis.runstatus,dis.runtimeefficiency,dis.runtime,dis.runrange "
											+" from tbl_rpc_diagram_hist t ,tbl_wellinformation t007 ,tbl_rpc_productiondata_hist prod,tbl_rpc_discrete_hist dis"
											+" where t.wellid=t007.id and t.productiondataid=prod.id and t.discretedataid=dis.id "
											+" and t.resultstatus=1 "
											+" and t.acqTime > to_date('"+endAcqTime.split(" ")[0]+"','yyyy-mm-dd')"
