...
public class GraphicalUploadService<T> extends BaseService<T> {
	...
	public String getKafkaConfigWellList(String orgId,String wellName){
		StringBuffer result_json = new StringBuffer();
		String sql="select t.id,t.wellname from tbl_wellinformation t where t.orgid in ("+orgId+")";
		if(StringManagerUtils.isNotNull(wellName)){
			sql+=" and t.wellName='"+wellName+"'";
		}
		int totals=this.getTotalCountRows(sql);
		List<?> list = this.findCallSql(sql);
		String columns = "[{ \"header\":\"序号\",\"dataIndex\":\"id\",width:50 ,children:[] },{ \"header\":\"井名\",\"dataIndex\":\"wellName\" ,children:[] }]";
		result_json.append("{ \"success\":true,\"columns\":"+columns+",");
		result_json.append("\"totalCount\":"+totals+",");
		result_json.append("\"totalRoot\":[");
		for(int i=0;i<list.size();i++){
