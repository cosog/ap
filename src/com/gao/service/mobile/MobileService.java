...
public class MobileService<T> extends BaseService<T> {
	...
	public List<T> getOrganizationData(Class<Org> class1, String userAccount) {
		...
		if(StringManagerUtils.isNotNull(userAccount)){
			queryString = "SELECT {Org.*} FROM sc_org Org   "
					+ " start with Org.org_id=( select t2.user_orgid from sc_user t2 where t2.user_id='test' )   "
	...
	public String getPumpingRealtimeStatisticsDataByOrgName(String orgName,String liftingType){
		...
		String orgIdSql="";
		if(orgArr.length==1){
			orgIdSql="select t.org_id from sc_org t where t.org_name='"+orgName+"'";
		}else if(orgArr.length==2){
			orgIdSql="select t.org_id from sc_org t where t.org_name='"+orgArr[1]+"' and t.org_parent=( select t2.org_id from sc_org t2 where t2.org_name='"+orgArr[0]+"' )";
		}else if(orgArr.length==3){
			orgIdSql="select t.org_id from sc_org t where t.org_name='"+orgArr[2]+"' "
					+ "and t.org_parent=( select t2.org_id from sc_org t2 where t2.org_name='"+orgArr[1]+"' "
					+ "and t2.org_parent=(select t3.org_id from sc_org t3 where t3.org_name='"+orgArr[0]+"') )";
		}else if(orgArr.length==4){
			orgIdSql="select t.org_id from sc_org t where t.org_name='"+orgArr[3]+"' "
					+ "and t.org_parent=( select t2.org_id from sc_org t2 where t2.org_name='"+orgArr[2]+"' "
					+ "and t2.org_parent=(select t3.org_id from sc_org t3 where t3.org_name='"+orgArr[1]+"' "
					+ "and t3.org_parent=(select t4.org_id from sc_org t4 where t4.org_name='"+orgArr[0]+"')  ) )";
		}else if(orgArr.length==5){
			orgIdSql="select t.org_id from sc_org t where t.org_name='"+orgArr[4]+"' "
					+ "and t.org_parent=( select t2.org_id from sc_org t2 where t2.org_name='"+orgArr[3]+"' "
					+ "and t2.org_parent=(select t3.org_id from sc_org t3 where t3.org_name='"+orgArr[2]+"' "
					+ "and t3.org_parent=(select t4.org_id from sc_org t4 where t4.org_name='"+orgArr[1]+"' "
					+ "and t4.org_parent=(select t5.org_id from sc_org t5 where t5.org_name='"+orgArr[0]+"') )  ) )";
		}
		List<?> orgList=null;
		...
		String gkmcSql="select  t.gkmc ,count(id) from v_analysisrealtime t "
				+ " where  t.org_id in (SELECT u.org_id FROM sc_org u   start with u.org_id="+orgId+"   connect by u.org_parent= prior u.org_id) ";
